<div class="hero py-8">
    <div class="justify-center items-center text-center space-y-4">
        <h1 class="mb-5 text-5xl font-bold">F.A.Q.</h1>
        <div tabindex="0" class="collapse bg-base-200 hover:collapse-open">
            <div class="collapse-title text-xl font-medium">
                How much does a subscription cost?
            </div>
            <div class="collapse-content">
                <p>
                    The full version of our project is absolutely free</p>
            </div>
        </div>
        <div tabindex="0" class="collapse bg-base-200 hover:collapse-open">
            <div class="collapse-title text-xl font-medium">
                I want to customize the project for myself
            </div>
            <div class="collapse-content">
                <p>No problem! our project is open source and located on <a href="https://github.com/FinTrackhq/FinTrack" class="link link-primary">Github</a></p>
            </div>
        </div>
        <div tabindex="0" class="collapse bg-base-200 hover:collapse-open">
            <div class="collapse-title text-xl font-medium">
                Can I display the number of employees on my website?
            </div>
            <div class="collapse-content">
                <p>YES! and not only the number of employees, our project has a fairly well-developed API and documentation for it will be released soon</p>
            </div>
        </div>

        <div tabindex="0" class="collapse bg-base-200 hover:collapse-open">
            <div class="collapse-title text-xl font-medium">
                I'm a developer. Can I participate in this project?
            </div>
            <div class="collapse-content">
                <p>Yes! you can help develop our project on <a href="https://github.com/FinTrackhq/FinTrack" class="link link-primary">Github</a> by creating a Pull request</p>
            </div>
        </div>
    </div>
</div>
